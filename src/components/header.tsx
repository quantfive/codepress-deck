...
export default function Header() {
  ...
  return (
    <header
      ref={headerRef}
      className={cn(
        "bg-white w-full sticky top-0 z-50 transition-transform duration-200 rounded-lg",
        isHeaderVisible ? undefined : "-translate-y-full"
      )}
    >
      <div className="max-w-screen-xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isMenuOpen ? (
              <X className="text-black size-6" aria-hidden="true" />
            ) : (
              <Menu className="text-black size-6" aria-hidden="true" />
            )}
          </Button>
        </div>
      </div>
